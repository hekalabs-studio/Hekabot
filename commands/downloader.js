...
function videoCommand(name, endpointKey, aliases = []) {
  return {
    ...
    run: async ({ jid, sock, args, text, reply }) => {
      const link = getLink(args, text);
      if (!link) return reply(`Kirim link-nya juga ya.\nContoh: *${name} https://...*`);
      const res = await apiGet(endpointKey, { url: link });
      const urls = findUrls(res);
      if (!urls.length) return reply("Gagal ambil video, link tidak valid atau API sedang bermasalah.");
...
function audioCommand(name, endpointKey, aliases = []) {
  return {
    name,
    aliases,
    run: async ({ jid, sock, args, text, reply }) => {
      ...
      const res = await apiGet(endpointKey, { url: link });
      const urls = findUrls(res);
      if (!urls.length) return reply("Gagal ambil audio, link tidak valid atau API sedang bermasalah.");
      const title = findText(res) || name;
      await reply({ audio: { url: urls[0] }, mimetype: "audio/mpeg", fileName: `${title}.mp3` });
    },
  };
}
...
function imageCommand(name, endpointKey, aliases = []) {
  return {
    ...
    run: async ({ jid, sock, args, text, reply }) => {
      ...
      if (!urls.length) return reply("Gagal ambil gambar, link tidak valid atau API sedang bermasalah.");
      for (const u of urls) await reply({ image: { url: u } });
    },
  };
}

function documentCommand(name, endpointKey, aliases = []) {
  return {
    name,
    aliases,
    run: async ({ jid, sock, args, text, reply }) => {
      ...
      const res = await apiGet(endpointKey, { url: link });
      const urls = findUrls(res);
      if (!urls.length) return reply("Gagal ambil file, link tidak valid atau API sedang bermasalah.");
      const title = findText(res) || name;
      await reply({ document: { url: urls[0] }, mimetype: "application/pdf", fileName: `${title}.pdf` });
    },
  };
}
...
module.exports = [
  ...
  videoCommand("rednotedl", "rednotedl"),
  documentCommand("scribddl", "scribddl"),
  documentCommand("slidesharedl", "slidesharedl"),
  audioCommand("spotifydl", "spotifydl"), // catatan: Spotify DRM, kemungkinan besar gak akan pernah jalan gratis
  imageCommand("telesticker", "telesticker"),
  videoCommand("teradl", "teradl"),
  videoCommand("teraview", "teraview"),
];
