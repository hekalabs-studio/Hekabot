...
// Deteksi emoji (termasuk kombinasi ZWJ, variation selector, skin-tone modifier) buat
// dipisahin dari teks biasa -- dipakai KHUSUS di overlayMemeText() (.smeme) buat nge-fix
// emoji yang sebelumnya render jadi kotak/garis hitam (lihat renderEmojiGlyph() di
// lib/bratCanvas.js buat penjelasan lengkap kenapa & caranya).
const EMOJI_REGEX = /\p{Extended_Pictographic}(?:\uFE0F|[\u{1F3FB}-\u{1F3FF}])?(?:\u200D\p{Extended_Pictographic}(?:\uFE0F|[\u{1F3FB}-\u{1F3FF}])?)*/gu;
...
async function overlayMemeText(imageBuffer, { top = "", bottom = "" } = {}) {
  ...
  function buildBlock(text) {
    ...
    let fontSize = Math.floor(Math.max(w, h) / 5.3); // diperbesar dari /6.5 -- request user, font .smeme dirasa kekecilan
  ...
  // renderBlock ASLI -- SAMA PERSIS kayak sebelumnya, TIDAK DIUBAH SAMA SEKALI. Ini yang tetap
  // dipakai kalau caption-nya gak ada emoji sama sekali (mayoritas kasus), jadi behavior/hasil
  // visualnya 100% identik kayak sebelum fix ini ada -- font/style/stroke gak ada yang berubah.
  ...
  // renderBlockWithEmoji -- CUMA dipakai kalau block-nya BENERAN ada emoji. Font-family, font-size,
  // fill, stroke, stroke-width, font-weight buat bagian TEKS-nya PERSIS SAMA kayak renderBlock()
  // di atas (gak ada satupun yang diubah) -- yang beda cuma emoji-nya digambar terpisah sebagai
  // <image> (dari renderEmojiGlyph(), lewat worker thread yang sama kayak .brat) di posisi yang
  // pas, bukan ikut nebeng di dalam <text> yang sama (itu yang bikin jadi kotak/garis hitam
  // sebelumnya, karena librsvg gak nemu font emoji yang valid di sistemnya).
  async function renderBlockWithEmoji(block, anchorY, direction, emojiDataUris) {
    const { line, fontSize } = block;
    const strokeWidth = Math.max(1.5, fontSize / 22);
    const y = direction === "down" ? anchorY + fontSize : anchorY;
    const emojiSize = fontSize * 0.95;

    const segments = splitEmojiSegments(line);
    const { measureTextAsync } = require("./bratCanvasAsync");

    // Ukur lebar PRESISI (pixel beneran, lewat canvas) buat tiap segmen teks -- BUKAN pakai
    // estimateCharWidth() (itu perkiraan kasar dikalibrasi buat Arial biasa, ternyata kelebaran
    // buat Impact/Arial Black yang notabene font RAMPING walau tebal -- itu yang bikin emoji
    // ke-dorong kejauhan dari teksnya sebelumnya). Segmen emoji ukurannya udah pasti (emojiSize),
    // gak perlu diukur.
    const segmentWidths = await Promise.all(
      segments.map((seg) => (seg.type === "emoji" ? emojiSize * 1.05 : measureTextAsync(seg.value, fontSize)))
    );
    ...
    const parts = segments.map((seg, i) => {
      ...
      let svgPart;
      if (seg.type === "emoji") {
        // y - fontSize*0.85 = perkiraan biar emoji sejajar vertikal sama cap-height teks
        // di sebelahnya (gak ada API buat ngukur cap-height asli font Impact dari Node,
        // jadi ini nilai perkiraan yang divisualkan cocok buat teks caption bold ala meme).
        svgPart = `<image x="${x}" y="${y - fontSize * 0.85}" width="${emojiSize}" height="${emojiSize}" href="${emojiDataUris.get(seg.value)}" />`;
      } else {
        svgPart = `<text x="${x}" y="${y}" font-family="Impact, 'Arial Black', 'Liberation Sans', sans-serif"
                     font-size="${fontSize}" fill="white" stroke="black" stroke-width="${strokeWidth}"
                     stroke-linejoin="round" font-weight="900">${escapeXml(seg.value)}</text>`;
      }
      x += segWidth;
      return svgPart;
    });
  ...
  // Kumpulin SEMUA emoji unik (dari top+bottom sekaligus) yang perlu di-render lewat worker
  // thread -- di-render SEKALI per karakter unik (bukan render ulang tiap kemunculan), terus
  // dipakai berkali-kali dari hasil yang udah jadi (lebih efisien kalau emoji yang sama
  // dipakai beberapa kali).
  const allEmoji = new Set();
  for (const block of [topBlock, bottomBlock]) {
    if (!block) continue;
    for (const seg of splitEmojiSegments(block.line)) {
      if (seg.type === "emoji") allEmoji.add(seg.value);
    }
  }
  ...
  if (allEmoji.size === 0) {
    // Gak ada emoji sama sekali -- jalur ASLI, gak disentuh, gak ada overhead tambahan.
    topSvg = renderBlock(topBlock, padding, "down");
    bottomSvg = renderBlock(bottomBlock, h - padding, "up");
  } else {
    const { renderEmojiGlyphAsync } = require("./bratCanvasAsync");
    const emojiList = [...allEmoji];
    const emojiBuffers = await Promise.all(emojiList.map((e) => renderEmojiGlyphAsync(e, 128)));
    const emojiDataUris = new Map(
      emojiList.map((e, i) => [e, `data:image/png;base64,${emojiBuffers[i].toString("base64")}`])
    );

    topSvg = topBlock
      ? (splitEmojiSegments(topBlock.line).some((s) => s.type === "emoji")
          ? await renderBlockWithEmoji(topBlock, padding, "down", emojiDataUris)
          : renderBlock(topBlock, padding, "down"))
      : "";
    bottomSvg = bottomBlock
      ? (splitEmojiSegments(bottomBlock.line).some((s) => s.type === "emoji")
          ? await renderBlockWithEmoji(bottomBlock, h - padding, "up", emojiDataUris)
          : renderBlock(bottomBlock, h - padding, "up"))
      : "";
  }
