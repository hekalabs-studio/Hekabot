...
async function overlayMemeText(imageBuffer, { top = "", bottom = "" } = {}) {
  ...
  function renderBlockWithEmoji(block, anchorY, direction, emojiDataUris) {
    const { line, fontSize } = block;
    const strokeWidth = Math.max(1.5, fontSize / 22);
    const y = direction === "down" ? anchorY + fontSize : anchorY;
    ...
    // Hitung lebar tiap segmen (perkiraan, sama heuristiknya kayak yang udah dipakai di
    // tempat lain di file ini buat estimasi lebar teks) -- buat nentuin posisi X tiap
    // segmen supaya keseluruhan baris tetap KETENGAH (setara text-anchor="middle" versi asli).
    const segmentWidths = segments.map((seg) =>
      seg.type === "emoji" ? emojiSize * 1.05 : estimateCharWidth(fontSize) * seg.value.length
    );
  ...
  if (allEmoji.size === 0) {
    ...
    topSvg = topBlock
      ? (splitEmojiSegments(topBlock.line).some((s) => s.type === "emoji")
          ? renderBlockWithEmoji(topBlock, padding, "down", emojiDataUris)
          : renderBlock(topBlock, padding, "down"))
      : "";
    bottomSvg = bottomBlock
      ? (splitEmojiSegments(bottomBlock.line).some((s) => s.type === "emoji")
          ? renderBlockWithEmoji(bottomBlock, h - padding, "up", emojiDataUris)
