...
async function makeQuoteCardV2(senderName, text, avatarBuffer) {
  ...
  const bubbleGap = 18; // jarak antara tepi avatar dan tepi bubble, dijembatani sama ekor
  const bubblePaddingX = 28;
  const bubblePaddingY = 24;
  const nameFontSize = 38;
  const nameGap = 16; // jarak antara nama dan tepi atas bubble
  const nameColor = colorForName(senderName);

  const bubbleX = avatarX + avatarSize + bubbleGap;
  const maxBubbleWidth = canvasSize - bubbleX - 20;
  const maxBubbleHeight = canvasSize - 40 - nameFontSize - nameGap;

  const displayText = String(text);
  const maxNameWidth = canvasSize - (bubbleX + 6) - 16;
  const maxNameChars = Math.max(4, Math.floor(maxNameWidth / estimateCharWidth(nameFontSize)));
  ...
  // Layout dari atas ke bawah: nama di baris paling atas, terus bubble persis di bawahnya.
  // Avatar sengaja disejajarkan sama TEPI ATAS bubble (bukan di-tengah-tengahin ke tinggi
  // bubble) biar avatar "nempel pas" sama bubble chat-nya kayak di aplikasi chat pada umumnya.
  const nameTopRel = 0;
  const nameBaselineYRel = nameTopRel + nameFontSize * 0.85;
  const bubbleYRel = nameBaselineYRel + nameFontSize * 0.3 + nameGap;
  const avatarYRel = bubbleYRel;

  const groupTop = Math.min(nameTopRel, avatarYRel);
  const groupBottom = Math.max(avatarYRel + avatarSize, bubbleYRel + bubbleHeight);
  ...
  // Ekor kecil segitiga yang nunjuk balik ke arah avatar, nempel di tepi kiri-atas bubble
  const tailAttachY = bubbleY + Math.min(46, bubbleHeight * 0.32);
  const tailPath = `M ${bubbleX + 4} ${tailAttachY - 10}
    L ${avatarX + avatarSize - 12} ${tailAttachY - 22}
    L ${bubbleX + 4} ${tailAttachY + 26} Z`;

  const cardSvg = `<svg width="${canvasSize}" height="${canvasSize}" xmlns="http://www.w3.org/2000/svg">
    <text x="${bubbleX + 6}" y="${nameBaselineY}" font-family="Arial, Helvetica, sans-serif"
          font-size="${nameFontSize}" font-weight="700" fill="${nameColor}">${escapeXml(displayName)}</text>
