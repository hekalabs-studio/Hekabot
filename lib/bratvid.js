 ...
 */
async function makeBratVideo(text, { bgColor = "#FFFFFF" } = {}) {
  ...
  try {
    const [frameBuffers, lastBuf] = await Promise.all([
      Promise.all(progressivePrompts.map((prompt) => makeBratImageAsync(prompt, { bgColor }))),
      makeBratImageAsync(text, { bgColor }),
    ]);
